...
//"Differentiated Parabolic Wave" technique
 class DPWosc {
	...
	private void update(double sqP, double swP, double sqP1,double sqP2,double sP1, double sP2,double IIP) {
		double sqPrev=sqP; //for sawtooth wave
		double sawPhase=swP; //for sawtooth wave
		double sqPrev1=sqP1, sqPrev2=sqP2; //for square/triangle wave
		double sawPhase1=sP1,sawPhase2=sP2;
		double IntegInputprev=IIP; //for triangle wave
		}
	
	public double GenOsc(double midiNote, int sampleIndex, double SR, String type ) {
	...
	conv=(double) (midiNote-69)/12;
	pitch=440*(Math.pow(2,conv));
